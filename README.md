<html lang="en"><head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Thiago Duarte - Portfolio</title>
    <link rel="stylesheet" href="styles.css">
<link rel="stylesheet" href="styles.css"><style>[_nghost-bvw-c45] {
      display: block;
      min-height: 100vh;
      background-color: var(--background);
    }</style><style>.hero-section[_ngcontent-bvw-c46] {
  min-height: 100vh;
  background-color: #0a0a0f;
  position: relative;
  padding: 2rem;
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  overflow: hidden;
}
.hero-section[_ngcontent-bvw-c46]::before {
  content: "";
  position: absolute;
  top: 0;
  left: 0;
  right: 0;
  bottom: 0;
  background: radial-gradient(circle at center, rgba(0, 199, 255, 0.1) 0%, rgba(10, 10, 15, 0) 70%), linear-gradient(45deg, rgba(255, 0, 128, 0.05) 0%, rgba(10, 10, 15, 0) 70%);
  pointer-events: none;
  animation: pulseBackground 8s ease infinite;
}
.hero-section[_ngcontent-bvw-c46]   .content-container[_ngcontent-bvw-c46] {
  max-width: 1200px;
  width: 100%;
  text-align: center;
  padding: 2rem;
  display: flex;
  flex-direction: column;
  align-items: center;
}
.hero-section[_ngcontent-bvw-c46]   .hero-content[_ngcontent-bvw-c46]   h1[_ngcontent-bvw-c46] {
  margin-bottom: 1.5rem;
  line-height: 1.2;
}
.hero-section[_ngcontent-bvw-c46]   .hero-content[_ngcontent-bvw-c46]   h1[_ngcontent-bvw-c46]   .name[_ngcontent-bvw-c46] {
  display: block;
  font-size: 4rem;
  font-weight: bold;
  color: #00C7FF;
  margin-bottom: 0.5rem;
  text-shadow: 0 0 10px rgba(0, 199, 255, 0.5), 0 0 20px rgba(0, 199, 255, 0.3), 0 0 30px rgba(0, 199, 255, 0.2);
  animation: glow 2s infinite;
  position: relative;
}
.hero-section[_ngcontent-bvw-c46]   .hero-content[_ngcontent-bvw-c46]   h1[_ngcontent-bvw-c46]   .name[_ngcontent-bvw-c46]::before {
  content: attr(data-text);
  position: absolute;
  left: 2px;
  text-shadow: -2px 0 #ff00ff;
  top: 0;
  color: #00C7FF;
  overflow: hidden;
  clip: rect(0, 900px, 0, 0);
  animation: noise-anim 3s infinite linear alternate-reverse;
}
.hero-section[_ngcontent-bvw-c46]   .hero-content[_ngcontent-bvw-c46]   h1[_ngcontent-bvw-c46]   .role[_ngcontent-bvw-c46] {
  display: block;
  font-size: 3rem;
  color: #ffffff;
  text-shadow: 0 0 10px rgba(255, 255, 255, 0.3), 0 0 20px rgba(255, 255, 255, 0.2);
  position: relative;
  overflow: hidden;
  font-weight: 600;
}
.hero-section[_ngcontent-bvw-c46]   .hero-content[_ngcontent-bvw-c46]   .description[_ngcontent-bvw-c46] {
  font-size: 1.25rem;
  color: var(--text-secondary);
  max-width: 600px;
  margin: 2rem auto;
  line-height: 1.6;
  opacity: 0.9;
}
.hero-section[_ngcontent-bvw-c46]   .hero-content[_ngcontent-bvw-c46]   .cta-button[_ngcontent-bvw-c46]   .view-projects-btn[_ngcontent-bvw-c46] {
  background: #00C7FF;
  color: #0B1221;
  padding: 0.75rem 2rem;
  border-radius: 4px;
  font-weight: 600;
  font-size: 1.125rem;
  transition: all 0.3s ease;
  border: none;
  cursor: pointer;
}
.hero-section[_ngcontent-bvw-c46]   .hero-content[_ngcontent-bvw-c46]   .cta-button[_ngcontent-bvw-c46]   .view-projects-btn[_ngcontent-bvw-c46]:hover {
  background: #33D3FF;
  transform: translateY(-2px);
}
.hero-section[_ngcontent-bvw-c46]   .featured-projects[_ngcontent-bvw-c46] {
  margin-top: 6rem;
  padding: 2rem;
}
.hero-section[_ngcontent-bvw-c46]   .featured-projects[_ngcontent-bvw-c46]   h2[_ngcontent-bvw-c46] {
  color: var(--cyber-primary);
  font-size: 2.5rem;
  margin-bottom: 3rem;
  text-align: left;
}
.hero-section[_ngcontent-bvw-c46]   .featured-projects[_ngcontent-bvw-c46]   .project-grid[_ngcontent-bvw-c46] {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(300px, 1fr));
  gap: 2rem;
}
.hero-section[_ngcontent-bvw-c46]   .featured-projects[_ngcontent-bvw-c46]   .project-grid[_ngcontent-bvw-c46]   .project-card[_ngcontent-bvw-c46] {
  background: rgba(11, 18, 33, 0.8);
  border: 1px solid rgba(0, 199, 255, 0.2);
  border-radius: 12px;
  padding: 2rem;
  transition: all 0.3s ease;
}
.hero-section[_ngcontent-bvw-c46]   .featured-projects[_ngcontent-bvw-c46]   .project-grid[_ngcontent-bvw-c46]   .project-card[_ngcontent-bvw-c46]   h3[_ngcontent-bvw-c46] {
  color: var(--cyber-primary);
  font-size: 1.5rem;
  margin-bottom: 1rem;
}
.hero-section[_ngcontent-bvw-c46]   .featured-projects[_ngcontent-bvw-c46]   .project-grid[_ngcontent-bvw-c46]   .project-card[_ngcontent-bvw-c46]   p[_ngcontent-bvw-c46] {
  color: var(--cyber-text);
  margin-bottom: 1.5rem;
}
.hero-section[_ngcontent-bvw-c46]   .featured-projects[_ngcontent-bvw-c46]   .project-grid[_ngcontent-bvw-c46]   .project-card[_ngcontent-bvw-c46]   .tech-tags[_ngcontent-bvw-c46] {
  display: flex;
  flex-wrap: wrap;
  gap: 0.5rem;
  margin-bottom: 1.5rem;
}
.hero-section[_ngcontent-bvw-c46]   .featured-projects[_ngcontent-bvw-c46]   .project-grid[_ngcontent-bvw-c46]   .project-card[_ngcontent-bvw-c46]   .tech-tags[_ngcontent-bvw-c46]   .tech-tag[_ngcontent-bvw-c46] {
  background: rgba(0, 199, 255, 0.1);
  color: var(--cyber-primary);
  padding: 0.25rem 0.75rem;
  border-radius: 4px;
  font-size: 0.875rem;
}
.hero-section[_ngcontent-bvw-c46]   .featured-projects[_ngcontent-bvw-c46]   .project-grid[_ngcontent-bvw-c46]   .project-card[_ngcontent-bvw-c46]   .view-project[_ngcontent-bvw-c46] {
  color: var(--cyber-primary);
  text-decoration: none;
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
}
.hero-section[_ngcontent-bvw-c46]   .featured-projects[_ngcontent-bvw-c46]   .project-grid[_ngcontent-bvw-c46]   .project-card[_ngcontent-bvw-c46]   .view-project[_ngcontent-bvw-c46]:hover {
  text-decoration: underline;
}
.hero-section[_ngcontent-bvw-c46]   .featured-projects[_ngcontent-bvw-c46]   .project-grid[_ngcontent-bvw-c46]   .project-card[_ngcontent-bvw-c46]:hover {
  transform: translateY(-5px);
  border-color: var(--cyber-primary);
  box-shadow: 0 0 30px rgba(0, 199, 255, 0.1);
}
.hero-section[_ngcontent-bvw-c46]::before {
  content: "";
  position: absolute;
  top: 0;
  left: 0;
  right: 0;
  bottom: 0;
  background: radial-gradient(circle at center, rgba(0, 199, 255, 0.1) 0%, transparent 70%);
  pointer-events: none;
}
.hero-section[_ngcontent-bvw-c46]   .title[_ngcontent-bvw-c46] {
  font-size: 4rem;
  color: var(--cyber-primary);
  margin-bottom: 0.5rem;
  font-weight: 700;
  animation: fadeInDown 0.8s ease-out;
  letter-spacing: -0.02em;
  text-shadow: 0 0 10px rgba(0, 199, 255, 0.5);
}
.hero-section[_ngcontent-bvw-c46]   .subtitle[_ngcontent-bvw-c46] {
  font-size: 1.25rem;
  color: var(--cyber-text);
  margin-bottom: 2rem;
  animation: fadeInUp 0.8s ease-out 0.2s backwards;
  font-weight: 400;
  line-height: 1.5;
  text-shadow: 0 0 5px rgba(203, 213, 225, 0.3);
}
.hero-section[_ngcontent-bvw-c46]   .cli-output[_ngcontent-bvw-c46] {
  font-family: "IBM Plex Mono", monospace;
  font-size: 0.875rem;
  color: var(--cyber-primary);
  text-shadow: 0 0 5px rgba(0, 199, 255, 0.5);
}
.content-container[_ngcontent-bvw-c46] {
  max-width: 800px;
  margin: 0 auto;
  text-align: center;
}
.button-group[_ngcontent-bvw-c46] {
  display: flex;
  gap: 1rem;
  justify-content: center;
}
.cyber-button[_ngcontent-bvw-c46] {
  @apply px-8 py-3 rounded relative overflow-hidden transition-all duration-300;
}
.cyber-button.primary[_ngcontent-bvw-c46] {
  @apply bg-cyber-primary text-cyber-bg;
}
.cyber-button.primary[_ngcontent-bvw-c46]:hover {
  box-shadow: 0 0 25px var(--cyber-primary);
  transform: translateY(-2px);
}
.cyber-button.outline[_ngcontent-bvw-c46] {
  @apply border-2 border-cyber-primary text-cyber-primary;
}
.cyber-button.outline[_ngcontent-bvw-c46]:hover {
  @apply bg-cyber-primary text-cyber-bg;
  transform: translateY(-2px);
}
@keyframes fadeInDown {
  from {
    opacity: 0;
    transform: translateY(-20px);
  }
  to {
    opacity: 1;
    transform: translateY(0);
  }
}
@keyframes fadeInUp {
  from {
    opacity: 0;
    transform: translateY(20px);
  }
  to {
    opacity: 1;
    transform: translateY(0);
  }
}
@keyframes scan-line {
  0% {
    transform: translateY(-100%);
  }
  100% {
    transform: translateY(100vh);
  }
}
@keyframes blink {
  0%, 100% {
    opacity: 1;
  }
  50% {
    opacity: 0;
  }
}
/*# sourceMappingURL=data:application/json;base64, */</style><style>@charset "UTF-8";
.projects-section[_ngcontent-bvw-c47] {
  padding: 4rem 2rem;
  background: var(--cyber-bg);
  position: relative;
  overflow: hidden;
}
.projects-section[_ngcontent-bvw-c47]::before {
  content: "";
  position: absolute;
  top: 0;
  left: 0;
  right: 0;
  bottom: 0;
  background: radial-gradient(circle at center, rgba(0, 199, 255, 0.15) 0%, transparent 70%);
  pointer-events: none;
}
.projects-section[_ngcontent-bvw-c47]   h2[_ngcontent-bvw-c47] {
  color: var(--cyber-primary);
  font-size: 2.5rem;
  margin-bottom: 3rem;
  text-align: center;
  font-weight: 600;
  text-shadow: var(--cyber-glow);
}
.project-cards[_ngcontent-bvw-c47] {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(300px, 1fr));
  gap: 2rem;
  max-width: 1200px;
  margin: 0 auto;
}
.project-card[_ngcontent-bvw-c47] {
  background: rgba(15, 23, 42, 0.9);
  padding: 2rem;
  border-radius: 12px;
  border: 1px solid var(--cyber-primary);
  box-shadow: var(--cyber-shadow);
  -webkit-backdrop-filter: blur(12px);
          backdrop-filter: blur(12px);
  transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);
  position: relative;
  overflow: hidden;
  display: flex;
  flex-direction: column;
  min-height: 400px;
  justify-content: space-between;
}
.project-card[_ngcontent-bvw-c47]:hover {
  transform: translateY(-5px);
  box-shadow: var(--cyber-shadow-lg);
}
.project-content[_ngcontent-bvw-c47] {
  flex: 1;
  display: flex;
  flex-direction: column;
}
.project-tags[_ngcontent-bvw-c47] {
  margin-top: auto;
  display: flex;
  flex-wrap: wrap;
  gap: 0.5rem;
  padding-top: 1rem;
  position: relative;
}
.project-tags[_ngcontent-bvw-c47]::before {
  content: "";
  position: absolute;
  inset: 0;
  background: radial-gradient(circle at top right, rgba(0, 199, 255, 0.15), transparent 70%);
  opacity: 0;
  transition: opacity 0.3s ease;
}
.project-tags[_ngcontent-bvw-c47]:hover {
  transform: translateY(-8px);
  border-color: var(--cyber-primary);
  box-shadow: var(--cyber-glow), 0 12px 48px rgba(0, 199, 255, 0.2);
}
.project-tags[_ngcontent-bvw-c47]:hover::before {
  opacity: 1;
}
.project-tags[_ngcontent-bvw-c47]:hover   h3[_ngcontent-bvw-c47] {
  color: var(--cyber-primary);
  text-shadow: var(--cyber-glow);
}
.project-tags[_ngcontent-bvw-c47]   h3[_ngcontent-bvw-c47] {
  color: var(--cyber-text);
  font-size: 1.5rem;
  margin-bottom: 1rem;
  font-weight: 600;
  transition: all 0.3s ease;
}
.project-tags[_ngcontent-bvw-c47]   p[_ngcontent-bvw-c47] {
  color: var(--cyber-text);
  margin-bottom: 1.5rem;
  line-height: 1.6;
}
.project-tags[_ngcontent-bvw-c47]   .tech-tags[_ngcontent-bvw-c47] {
  display: flex;
  flex-wrap: wrap;
  gap: 0.5rem;
  margin-top: auto;
  justify-content: center;
  padding-top: 1.5rem;
}
.project-tags[_ngcontent-bvw-c47]   .tech-tags[_ngcontent-bvw-c47]   .tech-tag[_ngcontent-bvw-c47] {
  background: rgba(0, 199, 255, 0.1);
  color: var(--cyber-primary);
  padding: 0.5rem 1rem;
  border-radius: 4px;
  font-size: 0.875rem;
  border: 1px solid rgba(0, 199, 255, 0.2);
  transition: all 0.3s ease;
}
.project-tags[_ngcontent-bvw-c47]   .tech-tags[_ngcontent-bvw-c47]   .tech-tag[_ngcontent-bvw-c47]:hover {
  background: rgba(0, 199, 255, 0.2);
  transform: translateY(-2px);
  box-shadow: var(--cyber-glow);
}
.project-tags[_ngcontent-bvw-c47]   .view-project[_ngcontent-bvw-c47] {
  color: var(--cyber-primary);
  text-decoration: none;
  font-weight: 500;
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
  transition: all 0.3s ease;
}
.project-tags[_ngcontent-bvw-c47]   .view-project[_ngcontent-bvw-c47]:hover {
  transform: translateX(4px);
  text-shadow: var(--cyber-glow);
}
.project-tags[_ngcontent-bvw-c47]   .view-project[_ngcontent-bvw-c47]::after {
  content: "→";
  transition: transform 0.3s ease;
}
.project-tags[_ngcontent-bvw-c47]   .view-project[_ngcontent-bvw-c47]:hover::after {
  transform: translateX(4px);
}
.tech-tags[_ngcontent-bvw-c47] {
  display: flex;
  flex-wrap: wrap;
  gap: 0.5rem;
  margin-bottom: 1.5rem;
}
.tech-tags[_ngcontent-bvw-c47]   .tech-tag[_ngcontent-bvw-c47] {
  background: rgba(16, 185, 129, 0.1);
  color: var(--vibrant-green);
  padding: 0.5rem 1rem;
  border-radius: 4px;
  font-size: 0.875rem;
  border: 1px solid rgba(16, 185, 129, 0.2);
}
.view-project[_ngcontent-bvw-c47] {
  color: var(--vibrant-green);
  text-decoration: none;
  font-weight: 500;
  display: inline-flex;
  align-items: center;
  transition: all 0.3s ease;
}
.view-project[_ngcontent-bvw-c47]:hover {
  transform: translateX(4px);
  text-shadow: var(--glow-effect);
}
/*# sourceMappingURL=data:application/json;base64, */</style><style>.skills-container[_ngcontent-bvw-c48] {
  max-width: 800px;
  margin: 2rem auto;
  padding: 2rem;
  background: var(--bg-glass);
  -webkit-backdrop-filter: blur(10px);
          backdrop-filter: blur(10px);
  border: 1px solid rgba(0, 199, 255, 0.1);
  border-radius: 8px;
  box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3);
}

.skills-grid[_ngcontent-bvw-c48] {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(300px, 1fr));
  gap: 2rem;
}

@media (max-width: 768px) {
  .skills-grid[_ngcontent-bvw-c48] {
    grid-template-columns: 1fr;
  }
}

.skill-item[_ngcontent-bvw-c48] {
  margin-bottom: 1.5rem;
  opacity: 0;
  animation: skillFadeIn 0.5s ease-out forwards;
  animation-delay: calc(var(--index) * 0.1s);
}

.skill-header[_ngcontent-bvw-c48] {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 0.5rem;
}

.skill-name[_ngcontent-bvw-c48] {
  font-family: "Fira Code", monospace;
  font-size: 1.1rem;
  color: var(--neon-primary);
  text-shadow: 0 0 5px var(--neon-primary);
  letter-spacing: 0.5px;
}

.skill-name[_ngcontent-bvw-c48]::before {
  content: "> ";
  opacity: 0.7;
  color: rgba(0, 199, 255, 0.7);
}

.skill-level[_ngcontent-bvw-c48] {
  color: var(--text-secondary);
  font-size: 0.9rem;
}

.skill-bar[_ngcontent-bvw-c48] {
  height: 4px;
  background: rgba(255, 255, 255, 0.1);
  border-radius: 2px;
  overflow: hidden;
  position: relative;
}

.skill-progress[_ngcontent-bvw-c48] {
  height: 100%;
  background: var(--neon-primary);
  box-shadow: 0 0 10px var(--neon-primary);
  transition: width 1s ease-in-out;
  position: relative;
}

.skill-progress[_ngcontent-bvw-c48]::after {
  content: "";
  position: absolute;
  top: 0;
  right: 0;
  width: 5px;
  height: 100%;
  background: #fff;
  filter: blur(3px);
  opacity: 0.8;
}

.skill-progress[_ngcontent-bvw-c48]::before {
  content: "";
  position: absolute;
  right: 0;
  top: 50%;
  transform: translateY(-50%);
  width: 4px;
  height: 8px;
  background: #fff;
  border-radius: 4px;
  box-shadow: 0 0 10px #00C7FF, 0 0 20px #00C7FF;
}

@keyframes skillFadeIn {
  from {
    opacity: 0;
    transform: translateY(10px);
  }
  to {
    opacity: 1;
    transform: translateY(0);
  }
}

@keyframes shimmer {
  0% {
    transform: translateX(-100%);
  }
  100% {
    transform: translateX(100%);
  }
}

@media (max-width: 768px) {
  .skills-container[_ngcontent-bvw-c48] {
    margin: 1rem;
    padding: 1.5rem;
  }

  .skill-name[_ngcontent-bvw-c48] {
    font-size: 0.9rem;
  }

  .skill-level[_ngcontent-bvw-c48] {
    font-size: 0.8rem;
  }
}
/*# sourceMappingURL=data:application/json;base64, */</style></head>
<body>
    <app-root _nghost-bvw-c45="" ng-version="11.2.14"><div _ngcontent-bvw-c45="" class="bg-[var(--background)] min-h-screen"><app-hero _ngcontent-bvw-c45="" _nghost-bvw-c46="" class="ng-tns-c46-0"><div _ngcontent-bvw-c46="" class="hero-section ng-tns-c46-0"><div _ngcontent-bvw-c46="" class="content-container ng-tns-c46-0 ng-trigger ng-trigger-terminalEffect"><div _ngcontent-bvw-c46="" class="hero-content ng-tns-c46-0"><h1 _ngcontent-bvw-c46="" class="ng-tns-c46-0"><span _ngcontent-bvw-c46="" class="name ng-tns-c46-0 ng-trigger ng-trigger-glitchText" style="transform: none;">Thiago Duarte</span><span _ngcontent-bvw-c46="" class="role ng-tns-c46-0 ng-trigger ng-trigger-typewriter">Software Developer</span></h1><p _ngcontent-bvw-c46="" class="description ng-tns-c46-0 ng-trigger ng-trigger-typewriter">Desenvolvedor Full Stack especializado em Java/Spring Boot e Angular, criando aplicações modernas e escaláveis com práticas DevOps e soluções cloud-native.</p></div><div _ngcontent-bvw-c46="" class="social-links ng-tns-c46-0"><a _ngcontent-bvw-c46="" href="https://github.com/thiagoduarte" target="_blank" class="social-link ng-tns-c46-0"><i _ngcontent-bvw-c46="" class="fab fa-github ng-tns-c46-0"></i></a><a _ngcontent-bvw-c46="" href="https://linkedin.com/in/thiagoduarte" target="_blank" class="social-link ng-tns-c46-0"><i _ngcontent-bvw-c46="" class="fab fa-linkedin ng-tns-c46-0"></i></a><a _ngcontent-bvw-c46="" href="mailto:contato@thiagoduarte.dev" class="social-link ng-tns-c46-0"><i _ngcontent-bvw-c46="" class="fas fa-envelope ng-tns-c46-0"></i></a></div><div _ngcontent-bvw-c46="" class="featured-projects ng-tns-c46-0"><div _ngcontent-bvw-c46="" class="project-grid ng-tns-c46-0"><div _ngcontent-bvw-c46="" class="project-card ng-tns-c46-0 ng-star-inserted" style=""><h3 _ngcontent-bvw-c46="" class="ng-tns-c46-0">Cadastro Nacional de Pessoas Desaparecidas</h3><p _ngcontent-bvw-c46="" class="ng-tns-c46-0">Sistema nacional para registro e gestão de informações sobre pessoas desaparecidas, desenvolvido para o Ministério da Justiça e Segurança Pública.</p><div _ngcontent-bvw-c46="" class="tech-tags ng-tns-c46-0"><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Java 17</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Spring Boot</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Docker</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Swagger</span><!----></div><!----></div><div _ngcontent-bvw-c46="" class="project-card ng-tns-c46-0 ng-star-inserted" style=""><h3 _ngcontent-bvw-c46="" class="ng-tns-c46-0">Evolução do Projeto - Sistema de Gestão para Petshop</h3><p _ngcontent-bvw-c46="" class="ng-tns-c46-0">Sistema avançado de gestão para pet shop com arquitetura de microserviços, implementando práticas modernas de DevOps e cloud computing.</p><div _ngcontent-bvw-c46="" class="tech-tags ng-tns-c46-0"><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Docker</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Kubernetes</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">AWS</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Jenkins</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Microservices</span><!----></div><!----></div><div _ngcontent-bvw-c46="" class="project-card ng-tns-c46-0 ng-star-inserted" style=""><h3 _ngcontent-bvw-c46="" class="ng-tns-c46-0">Sistema de Gestão para Pizzaria</h3><p _ngcontent-bvw-c46="" class="ng-tns-c46-0">Sistema completo para gerenciamento de pedidos e operações de pizzaria, com interface intuitiva e banco de dados otimizado.</p><div _ngcontent-bvw-c46="" class="tech-tags ng-tns-c46-0"><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Java</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">MySQL</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Web Development</span><!----></div><!----></div><div _ngcontent-bvw-c46="" class="project-card ng-tns-c46-0 ng-star-inserted" style=""><h3 _ngcontent-bvw-c46="" class="ng-tns-c46-0">Matriz do Projeto - Sistema de Gestão para Petshop</h3><p _ngcontent-bvw-c46="" class="ng-tns-c46-0">Sistema integrado com emissão de notas fiscais e chatbot para WhatsApp, oferecendo automação de processos e melhor experiência ao cliente.</p><div _ngcontent-bvw-c46="" class="tech-tags ng-tns-c46-0"><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">NF-e</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">WhatsApp API</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">MySQL</span><!----></div><!----></div><div _ngcontent-bvw-c46="" class="project-card ng-tns-c46-0 ng-star-inserted" style=""><h3 _ngcontent-bvw-c46="" class="ng-tns-c46-0">Sistema de Gestão de Pedidos e Produção</h3><p _ngcontent-bvw-c46="" class="ng-tns-c46-0">Aplicação web moderna com relatórios personalizados e integração com WhatsApp para comunicação com clientes.</p><div _ngcontent-bvw-c46="" class="tech-tags ng-tns-c46-0"><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Angular</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Spring Boot</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">TypeScript</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">MySQL</span><!----></div><!----></div><div _ngcontent-bvw-c46="" class="project-card ng-tns-c46-0 ng-star-inserted" style=""><h3 _ngcontent-bvw-c46="" class="ng-tns-c46-0">Sistema de Gestão para Petshop (Microservices)</h3><p _ngcontent-bvw-c46="" class="ng-tns-c46-0">Versão modernizada do sistema de gestão utilizando arquitetura de microserviços e práticas DevOps para maior escalabilidade.</p><div _ngcontent-bvw-c46="" class="tech-tags ng-tns-c46-0"><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Docker</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Kubernetes</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">AWS</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Jenkins</span><!----></div><!----></div><div _ngcontent-bvw-c46="" class="project-card ng-tns-c46-0 ng-star-inserted" style=""><h3 _ngcontent-bvw-c46="" class="ng-tns-c46-0">Sistema de Gestão para Pizzaria - Versão Base</h3><p _ngcontent-bvw-c46="" class="ng-tns-c46-0">Sistema fundamental para gerenciamento de pedidos e operações de pizzaria com interface simplificada.</p><div _ngcontent-bvw-c46="" class="tech-tags ng-tns-c46-0"><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">MySQL</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Java</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Web Development</span><!----></div><!----></div><div _ngcontent-bvw-c46="" class="project-card ng-tns-c46-0 ng-star-inserted" style=""><h3 _ngcontent-bvw-c46="" class="ng-tns-c46-0">Sistema Genérico de Gestão</h3><p _ngcontent-bvw-c46="" class="ng-tns-c46-0">Sistema versátil para gestão de pedidos e produção com relatórios personalizados e integração com WhatsApp.</p><div _ngcontent-bvw-c46="" class="tech-tags ng-tns-c46-0"><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Angular</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">Spring Boot</span><span _ngcontent-bvw-c46="" class="tech-tag ng-tns-c46-0 ng-star-inserted">TypeScript</span><!----></div><!----></div><!----></div></div></div></div></app-hero><app-projects _ngcontent-bvw-c45="" _nghost-bvw-c47="" class="ng-tns-c47-1"><section _ngcontent-bvw-c47="" id="projetos" class="min-h-screen bg-[#1A1A2E] text-[#CBD5E1] p-8 ng-tns-c47-1"><div _ngcontent-bvw-c47="" class="max-w-6xl mx-auto ng-tns-c47-1"><div _ngcontent-bvw-c47="" class="grid md:grid-cols-2 gap-8 ng-tns-c47-1"></div></div></section></app-projects><app-skills _ngcontent-bvw-c45="" _nghost-bvw-c48="" class="ng-tns-c48-2"><section _ngcontent-bvw-c48="" aria-label="Skills Section" class="skills-container ng-tns-c48-2"><div _ngcontent-bvw-c48="" class="skills-grid ng-tns-c48-2"><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">Java</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">90%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="90"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 90%;"></div></div></div><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">Spring Boot</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">85%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="85"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 85%;"></div></div></div><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">Angular</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">85%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="85"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 85%;"></div></div></div><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">TypeScript</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">80%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="80"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 80%;"></div></div></div><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">Docker</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">75%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="75"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 75%;"></div></div></div><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">Kubernetes</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">70%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="70"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 70%;"></div></div></div><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">Jenkins</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">75%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="75"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 75%;"></div></div></div><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">Maven</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">80%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="80"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 80%;"></div></div></div><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">Hibernate</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">85%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="85"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 85%;"></div></div></div><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">JavaFX</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">75%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="75"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 75%;"></div></div></div><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">SCSS</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">80%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="80"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 80%;"></div></div></div><div _ngcontent-bvw-c48="" class="skill-item ng-tns-c48-2 ng-trigger ng-trigger-skillAnimation ng-star-inserted"><div _ngcontent-bvw-c48="" class="skill-header ng-tns-c48-2"><span _ngcontent-bvw-c48="" role="heading" aria-level="3" class="skill-name ng-tns-c48-2">Angular CLI</span><span _ngcontent-bvw-c48="" class="skill-level ng-tns-c48-2">85%</span></div><div _ngcontent-bvw-c48="" role="progressbar" aria-valuemin="0" aria-valuemax="100" class="skill-bar ng-tns-c48-2" aria-valuenow="85"><div _ngcontent-bvw-c48="" class="skill-progress ng-tns-c48-2 ng-trigger ng-trigger-progressAnimation" style="width: 85%;"></div></div></div><!----></div></section></app-skills></div></app-root>
<script src="runtime.js" type="module"></script><script src="polyfills.js" type="module"></script><script src="styles.js" type="module"></script><script src="vendor.js" type="module"></script><script src="main.js" type="module"></script>
</body></html>
